...
export class ImportReviewService {
  ...
  assignGoalOwner(
    _id: string,
    _program: Program,
    _input: Omit<AssignGoalOwnerRemediationInput, "importJobId"> & {
      expectedAnalysisRevision: number;
      expectedOldOwner?: string;
      actor: SessionUser;
      ownerDisplayName: string;
    },
    _sourceFinding: Record<string, unknown>,
    _sourceProvenance?: Record<string, unknown>
  ): ImportJob {
    throw new Error("Goal ownership is not part of the PULSE business model.");
  }
  ...
  approvalReadiness(id: string): ImportApprovalResult {
    ...
    if (!masterPlanSource && job.assessmentResult.governance.errors.some((violation) => this.isCriticalGovernanceViolation(violation.rule))) {
    ...
    if (!masterPlanSource && job.assessmentResult.findings.some((finding) =>

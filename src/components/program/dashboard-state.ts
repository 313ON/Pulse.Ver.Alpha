...
export type DashboardState =
  | { kind: "loading" }
  | { kind: "empty"; planYear: string }
  | { kind: "partial"; program: Program; missing: string[] }
  | { kind: "ready"; program: Program }
  | { kind: "recoverable-error"; message: string }
  | { kind: "blocking-error"; message: string; guidance: string };

export function classifyDashboardData(program: Program): Extract<DashboardState, { kind: "empty" | "partial" | "ready" }> {
  ...
  if (goals.length === 0) return { kind: "empty", planYear: program.timeline.start.split("/")[0] };
  if (actions.length === 0) missing.push("اقدام‌های متصل");
  if (goals.some((goal) => goal.objectives.length === 0)) missing.push("اهداف جزئی");
  if (missing.length > 0) return { kind: "partial", program, missing: [...new Set(missing)] };

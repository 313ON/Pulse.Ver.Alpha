...
export function StrategicCommandCenter() {
  const goalCount = programFixture.goals.length;
  const objectiveCount = programFixture.goals.reduce((total, goal) => total + goal.objectives.length, 0);
  const actionCount = programFixture.goals.reduce((total, goal) => total + goal.objectives.reduce((inner, objective) => total + objective.activities.reduce((activityTotal, activity) => total + activity.actions.length, 0), 0), 0);
    ...
    <div className="page strategic-command-center">
      <div className="page-heading strategic-heading">
        <div>
          ...
          <HierarchyBreadcrumb nodes={[programFixture]} />
        </div>
      ...
      <div className="strategic-summary">
        <div className="strategic-summary-main"><div><span className="program-panel-kicker">برنامه فعال</span><h2>{programFixture.title}</h2><p>{programFixture.description}</p></div><ProgressIndicator value={programFixture.progress} /></div>
